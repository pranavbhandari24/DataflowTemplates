...
public class PubsubToBigQueryLT extends TemplateLoadTestBase {
  ...
  public void testSpikyWorkload1hr(Function<LaunchConfig.Builder, LaunchConfig.Builder> paramsAdder)
      throws ParseException, IOException, InterruptedException {
    // Arrange
    TopicName inputTopic = pubsubResourceManager.createTopic("spiky-workload-input");
    SubscriptionName inputSubscription =
        pubsubResourceManager.createSubscription(inputTopic, "spiky-workload-subscription");
    TableId table =
        bigQueryResourceManager.createTable(
            testName, SCHEMA, System.currentTimeMillis() + 7200000); // expire in 2 hrs
    DataGenerator dataGenerator =
